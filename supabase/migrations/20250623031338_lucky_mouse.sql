...
-- Create the cohort retention analysis function with proper security and search path
...
BEGIN
  ...
  retained AS (
    ...
    FROM first_day f
    JOIN public.pageviews p USING (session_id)
...
COMMENT ON FUNCTION cohort_retention_analysis() IS 'Analyzes user retention by cohort day and day offset with fixed search path';