...
export const handler = async (event, context) => {
  ...
  try {
    ...
    let query = supabase
      .from(TABLES.ALERTS)
      .select('*')
      .order(COLUMNS.ALERTS.CREATED_AT, { ascending: false })
      .limit(limit)

    // Apply filters
    if (acknowledged !== null) {
      query = query.eq(COLUMNS.ALERTS.ACKNOWLEDGED, acknowledged === 'true')
    }

    if (severity) {
      query = query.eq(COLUMNS.ALERTS.SEVERITY, severity)
    }

    if (type) {
      query = query.eq(COLUMNS.ALERTS.TYPE, type)
    }
