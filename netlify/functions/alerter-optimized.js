...
async function checkAlertsRealtime(eventData) {
  ...
  try {
    ...
    console.log('🔍 Checking alerts via Realtime trigger (optimized)...')
...
async function checkAlertsOptimized() {
  ...
  try {
    console.log('🚀 Starting optimized alert checking...')
    ...
    console.log(`🔍 Querying events from: ${twoDaysAgo}`)
    ...
    while (hasMoreData && totalRecords < MAX_RECORDS && !earlyTerminated) {
      ...
      if (batchError) {
        console.error('❌ Database query error:', batchError)
        throw batchError
      }
      ...
      console.log(`📦 Processing batch ${processedBatches + 1}: ${batch.length} records (${batchQueryTime.toFixed(0)}ms)`)
      ...
      if (totalRecords >= MAX_RECORDS) {
        console.log(`🛑 Early termination: Reached ${MAX_RECORDS} record limit`)
      ...
      if (processedBatches >= 10) {
        console.log('⏸️ Memory efficiency pause after 10 batches')
        await new Promise(resolve => setTimeout(resolve, 100))
      }
    }

    performanceMetrics.recordsProcessed = totalRecords
    console.log(`📊 Processed ${totalRecords} total records in ${processedBatches} batches`)
    ...
    console.log(`✅ Optimized alert checking completed in ${totalTime.toFixed(0)}ms`)
    console.log(`📊 Performance: ${performanceMetrics.queriesExecuted} queries, ${performanceMetrics.recordsProcessed} records processed`)

  } catch (error) {
    console.error('❌ Optimized alert checking failed:', error)

    // Provide specific optimization suggestions based on error type
    if (error.message?.includes('Database query timeout')) {
      console.error('💡 Optimization Suggestion: Query timeout detected. Try:')
      ...
      console.error('💡 Optimization Suggestion: Memory issue detected. Try:')
...
async function getOptimizedForecast() {
  ...
  console.log(`🔮 Fetching optimized forecast from: ${forecastUrl}`)
...
async function checkSegmentForAlertOptimized(agg, forecast) {
  ...
  console.log(`📊 Alert check: ${segmentKey}`)
  ...
  sendAlertOptimized({
    ...
    console.error('❌ Async alert sending failed:', error)
  })
}
...
async function sendSlackAlertOptimized(alert) {
  ...
  if (slackResponse.ok) {
    ...
    console.error('❌ Slack alert failed:', slackResponse.status)
  }
}
...
const handler = async (event, context) => {
  ...
  try {
    ...
    console.log(`🚨 Optimized Alerter function triggered${isScheduled ? ' (scheduled)' : ' (manual)'}`)
    ...
    console.error('❌ Optimized Alerter function error:', error)
    return {
      statusCode: 500,
      headers: { 'Access-Control-Allow-Origin': '*' },
      body: JSON.stringify({
